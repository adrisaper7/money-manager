...
export const InvestmentPanel = ({ data, stats, updateData, language, exchangeRates }) => {
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Assets - Investment</h3>
                    ...
                    <tbody>
                        {rows.map(r => (
                            <tr key={r.category} className="border-t border-slate-100">
                                ...
                                <td className="py-3 text-right font-mono">{formatCurrency(r.value, 'en', exchangeRates)}</td>
                            </tr>
                        ))}
                        <tr className="border-t border-slate-200 font-semibold">
                            <td className="py-3">Total</td>
                            <td className="py-3 text-right font-mono">{formatCurrency(total, 'en', exchangeRates)}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div className="mt-4">
                <MonthlyInvestmentAllocator data={data} stats={stats} updateData={updateData} exchangeRates={exchangeRates} />
            </div>
        </div>
    );
};
