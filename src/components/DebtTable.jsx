...
export const DebtTable = ({
    ...
    const debtMetrics = useMemo(() => {
        if (!historicalData || historicalData.length === 0) {
            return { totalDebt: 0, totalPaid: 0, debtToAssetRatio: 0 };
        }
        ...
        // Show total amount paid from the debtCollaboration column
        const totalPaid = Object.values(currentMonth?.debtCollaboration || {}).reduce((a, b) => a + Number(b || 0), 0);

        const debtToAssetRatio = totalAssets > 0 ? (totalDebt / totalAssets) * 100 : 0;

        return { totalDebt, totalPaid, debtToAssetRatio };
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-200 rounded-lg p-3">
                    ...
                    <p className="text-lg font-bold text-emerald-600">
                        +{formatCurrency(debtMetrics.totalPaid, 'en', exchangeRates)}
                    </p>
                </div>
