...
export default function App() {
  ...
  const { data, updateData, addPreviousMonth, removeLastMonth, loadData } = useFireData(currentUser?.id);
  ...
  if (userLoading || languageLoading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'settings' && (
          <SettingsView config={config} setConfig={setConfig} stats={stats} t={t} language={language} exchangeRates={exchangeRates} />
        )}
      </main>
    </div>
  );
